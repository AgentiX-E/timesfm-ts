...
describe('forecastWithCovariates', () => {
  ...
  it('handles missing backcast in timesfm + xreg mode', async () => {
    // Mock a model that returns forecast without backcast
    const mockModel = {
      forecastConfig: model.forecastConfig,
      modelConfig: model.modelConfig,
      forecast: async () => ({
        pointForecast: [new Float32Array(8).fill(0)],
        quantileForecast: [Array.from({ length: 10 }, () => new Float32Array(8).fill(0))],
        backcast: undefined,
      }),
    } as any;
    ...
    const result = await forecastWithCovariates(mockModel, {
  ...
  it('handles short backcast in timesfm + xreg mode', async () => {
    // Mock where backcast exists but is shorter than training window
    const mockModel = {
      forecastConfig: model.forecastConfig,
      modelConfig: model.modelConfig,
      forecast: async () => ({
        pointForecast: [new Float32Array(8).fill(0)],
        quantileForecast: [Array.from({ length: 10 }, () => new Float32Array(8).fill(0))],
        backcast: [new Float32Array(0)], // empty backcast triggers : 0 ternary fallback
      }),
    } as any;
  ...
  it('handles singular design matrix gracefully (ridge fallback)', async () => {
    // Use static numerical covariate = 0 with ridge = 0 to make the
    // design matrix singular (covariate column all zeros), triggering
    // the catch block that increments the ridge penalty.
    const mockModel = {
      forecastConfig: model.forecastConfig,
      modelConfig: model.modelConfig,
      forecast: async () => ({
