 ...
 * coverage.  Only 0.8 (80%) is supported because the model does not
 * output finer-grained percentiles.
 */
 ...
 * at the given confidence level.  Only 80% CI is available from
 * TimesFM's 10-quantile output, which maps to Q10–Q90.
 *
 * @param output        Forecast output from `model.forecast()`.
 * @param seriesIndex   Index of the series (0-based).
 * @param confidence    Confidence level — only 0.8 is supported.
 *
 * @example
 * ```typescript
